...
{  
  ...
  FLA_Obj_create (FLA_DOUBLE_COMPLEX, output_states, input_states, 0, 0, &adjacency_matrix);
...
Arguments:
  ...
  FLA_Obj: Probability adjacency matrix. Contains resulting probability for each state transition
*/
void draw_transition_lines (Image *image, FLA_Obj A, int time_step, const int spacing_x, const int spacing_y)
{
    ...
    while (FLA_Obj_width( AL ) < FLA_Obj_width( A ) )
    {
      ...
      while (FLA_Obj_length(BT) < FLA_Obj_length(a1)) 
      {
        ...
        int curr_x_pos = GET_X_POS (time_step - 1) + RECS_HEIGHT; // Don't draw over previous state recs
        ...
        if (get_probability(beta) > 0.01) 
        {
          ...
          ImageDrawLineEx (image, (Vector2){curr_x_pos, curr_y_pos}, (Vector2){next_pos_x, next_pos_y}, thick, color);             
        }
      ...
      // ImageDrawLineV(image, (Vector2){curr_x_pos, curr_y_pos}, (Vector2){next_pos_x, next_pos_y}, RED);   
      // ImageDrawRectangleRec (image, rec, color);    
      // ImageDrawText (image, state_label, x_pos + RECS_HEIGHT * 0.40, y_pos + RECS_HEIGHT * 0.40, font_size, BLUE);
